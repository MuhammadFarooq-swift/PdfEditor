...
export const tabButtonRenderer: ComponentRenderFunction<TabButtonProps> = (
  ...
  return (
    <Button
      key={props.id}
      className={`rounded-none px-2 py-1 text-sm hover:bg-transparent ${active ? 'border-b-2 border-b-red-500 text-red-500' : 'border-b-2 border-b-transparent hover:border-b-gray-500'} hover:ring-transparent`}
      onClick={handleClick}
    >
      {props.label}
    </Button>
...
function HitLine({
  ...
  return (
    <button
      ref={ref}
      onClick={() => onClick(hit)}
      className={`shadow-xs w-full cursor-pointer overflow-hidden rounded border border-[#cfd4da] p-3 text-left text-xs leading-tight text-gray-600 ${active ? 'border-red-500 bg-red-50' : 'hover:border-[#1a466b] hover:bg-gray-100'}`}
    >
      {truncatedLeft && '… '}
      {before}
      ...
      <div className="p-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-md border border-gray-300 py-1 pl-8 pr-9 text-sm focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500"
          />
          {inputValue && (
            <div
    ...
    return (
      <Fragment key={command.id}>
        ...
        <button
          ...
          className={`flex cursor-pointer flex-row items-center justify-between gap-2 px-4 py-1 ${command.type === 'menu' ? 'menu-item' : ''} ${command.active && !command.disabled ? 'bg-red-500 text-white' : 'text-gray-500 hover:bg-red-900 hover:text-white'} ${command.disabled ? 'pointer-events-none cursor-not-allowed opacity-50' : ''} `}
        >
      ...
      {(m) => (
        ...
        >
          ...
          <div
            ...
            className={`outline outline-2 -outline-offset-2 ${props.currentPage === m.pageIndex + 1 ? 'outline-red-500' : 'outline-gray-300'}`}
          >
...
export const selectButtonRenderer: ComponentRenderFunction<SelectButtonProps> = (
  ...
  return (
    <div style={{ maxWidth: '100px', width: '100px' }}>
      <Button
        className={`col-start-1 row-start-1 !w-full appearance-none rounded-md bg-white py-1.5 pl-3 pr-2 text-[13px] text-gray-900 ${active ? 'text-red-500 outline outline-2 -outline-offset-2 outline-red-500' : 'outline outline-1 -outline-offset-1 outline-gray-300'} flex flex-row items-center justify-between gap-2 hover:ring-transparent`}
        onClick={handleClick}
      >
  ...
  return (
    <Dialog open={props.open} title="Print Settings" onClose={handleClose} maxWidth="28rem">
      <div className="space-y-6">
        ...
        {/* Loading indicator */}
        {isLoading && (
          <div className="flex items-center space-x-3 rounded-md bg-red-50 p-3">
            ...
            <div className="text-sm text-red-700">{loadingMessage}</div>
          </div>
        )}
        ...
        <div className="flex justify-end space-x-3 border-t border-gray-200 pt-4">
          ...
          <Button
            onClick={handlePrint}
            disabled={!canSubmit}
            className="flex items-center space-x-2 rounded-md border border-transparent bg-red-600 px-4 py-2 text-sm text-white hover:!bg-red-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isLoading && (
              <svg
